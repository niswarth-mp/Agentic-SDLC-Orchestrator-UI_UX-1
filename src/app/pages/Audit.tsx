    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">Complete activity trail across all projects and agents</p>
        </div>
          ...
          {auditEvents.map((event, index) => (
            <div key={event.id}>
              <div className="flex items-start gap-4 p-4 bg-white/5 hover:bg-white/10 rounded-lg transition-all">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h3 className="text-white font-medium mb-1">{event.action}</h3>
                      <p className="text-sm text-gray-400">{event.target}</p>
                    </div>
                    <span className={`px-3 py-1 text-xs rounded-full font-medium ${getTypeColor(event.type)}`}>
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-400">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[#6366F1] font-mono">{event.agent}</span>
                    </div>
                    <span>•</span>
                    <div className="flex items-center gap-2">
              ...
              {index < auditEvents.length - 1 && (
                <div className="ml-10 h-4 w-0.5 bg-white/10" />
              )}
            </div>
          ))}
        </div>
